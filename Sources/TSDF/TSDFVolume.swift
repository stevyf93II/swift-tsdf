...
//  swift-tsdf
//
//  CPU fixed-grid TSDF integration.
//
...
//  this and a naive accumulated point cloud.
//
//  A later phase ports the integrate kernel to Metal compute. This is pure
...
public struct VolumeConfig {
    public var voxelSize: Float = 0.015           // 15mm default
    public var truncation: Float = 0.06           // 4x voxelSize
    public var maxWeight: Float = 64              // caps running average
    public var minConfidence: UInt8 = 2           // .high only for v1
    public var depthRange: ClosedRange<Float> = 0.3...4.5  // sensor reliability bounds
...
public struct IntegrateStats {
    public var projected = 0            // voxels that projected into a valid depth pixel
    public var conf0 = 0               // of those, pixels with confidence 0 (low)
    public var conf1 = 0               // confidence 1 (medium)
    public var conf2 = 0               // confidence 2 (high)
    public var rejectedByConf = 0      // dropped: conf < minConfidence
    public var rejectedByDepthRange = 0 // dropped: depth NaN or outside depthRange
    public var rejectedBySDF = 0       // dropped: behind surface beyond -truncation
    public var voxelUpdates = 0        // voxels actually written

    public init() {}

    init(projected: Int, conf0: Int, conf1: Int, conf2: Int,
         rejectedByConf: Int, rejectedByDepthRange: Int,
         rejectedBySDF: Int, voxelUpdates: Int) {
        self.projected = projected
        self.conf0 = conf0
        self.conf1 = conf1
        self.conf2 = conf2
        self.rejectedByConf = rejectedByConf
        self.rejectedByDepthRange = rejectedByDepthRange
        self.rejectedBySDF = rejectedBySDF
        self.voxelUpdates = voxelUpdates
    }

    public static func + (a: IntegrateStats, b: IntegrateStats) -> IntegrateStats {
        IntegrateStats(
...
public struct Voxel {
    public var tsdf: Float = 0
    public var weight: Float = 0
    public var color: SIMD3<UInt8> = SIMD3<UInt8>(0, 0, 0)
...
public final class TSDFVolume {
    public let dim: Int                    // grid resolution
    public let origin: SIMD3<Float>        // world position of voxel (0,0,0) CORNER
    public let voxelSize: Float
    public let truncation: Float
    public let maxWeight: Float
    public let minConfidence: UInt8
    public let depthRange: ClosedRange<Float>

    /// The raw voxel grid, indexed via `index(_:_:_:)`. Exposed mutable on
    /// purpose: analytic fills (tests, SDF composition) write it directly.
    public var voxels: [Voxel]

    public init(dim: Int, origin: SIMD3<Float>, config: VolumeConfig) {
        self.dim = dim
        self.origin = origin
    ...
    @inline(__always)
    public func worldOfVoxel(_ x: Int, _ y: Int, _ z: Int) -> SIMD3<Float> {
        return SIMD3<Float>(
    ...
    ///
    /// depth/confidence dims expected: depthWidth x depthHeight.
