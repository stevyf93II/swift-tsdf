...
//  swift-tsdf
//
//  Walk each cube of 8 neighboring voxels in the TSDF volume, classify by
//  sign pattern, interpolate vertex positions along crossed edges using the
//  TSDF magnitudes for sub-voxel accuracy, emit triangles per the standard
//  Lorensen-Cline table.
//
//  Output: unwelded vertices (3 per triangle). A later pass can add welding.
...
public enum MarchingCubes {
    ...
    public static func extract(from volume: TSDFVolume) -> Mesh {
        let dim = volume.dim
